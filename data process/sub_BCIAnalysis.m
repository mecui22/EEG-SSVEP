...
% ITR
h = figure;
% plot(time/1000,mean(itr_trca,2),'m','LineWidth',2)
% hold on
plot(time/1000,mean(itr_fbcca,2),'r','LineWidth',2);
hold on
[maxvalue,index] = max(itr_fbcca);
txtfb = ['Max = ',num2str(maxvalue,'%.2f')];
scatter(index*0.2,maxvalue,100,'r');
plot(nan(size(time)),'w');
plot(time/1000,mean(itr_cca,2),'b','LineWidth',2);
[maxvalue,index] = max(itr_cca);
txtcca = ['Max = ',num2str(maxvalue,'%.2f')];
scatter(index*0.2,maxvalue,100,'b');
...
legend('FBCCA',txtfb,'','CCA',txtcca,'Location','northeastoutside');
legend('boxoff');
set(gca,'xlim',[0,3]);
...
saveas(h,[savepath,'S',num2str(sub),'-4-ITR.bmp'])
close(h)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end